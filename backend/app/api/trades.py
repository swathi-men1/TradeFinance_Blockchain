...
def create_trade(
    ...
    - **BANK** users initiate trades between corporate users (as facilitator)
    - **CORPORATE** users can create trades where they are buyer or seller
    - **ADMIN** can create trades for system administration/corrections only
    - **AUDITOR** cannot create trades (read-only)
    
    Domain Model:
    - Initiated by: Bank User (as operator) or Corporate User (as participant)
    - Buyer: Corporate User (participant subject to risk evaluation)
    - Seller: Corporate User (participant subject to risk evaluation)
    """
...
def list_trades(
    ...
    - **BANK**: All trades (as operator/initiator)
    - **CORPORATE**: Only trades where they are buyer or seller (as participant)
    - **AUDITOR**: All trades (read-only access)
    - **ADMIN**: All trades (system management)
    """
...
def update_trade(
    ...
    """
    Update trade details (Admin only - system corrections/fixes).
    
    Admin can update trade details for system administration purposes:
    - Correcting data entry errors
    - Fixing invalid buyer/seller assignments
    - Reassigning participants
    
    This is NOT for regular trade operations - use status endpoint for workflow.
    Note: Both buyer and seller must always be CORPORATE users.
    """
