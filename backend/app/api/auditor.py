...
def verify_document(
    ...
    """
    Verify document integrity by recalculating SHA-256 hash (read-only audit).
    
    - **AUDITOR** only
    - Downloads document from storage and recalculates SHA-256 hash
    - Compares with stored hash to detect tampering
    - Creates ledger entry recording the verification action
    - Creates compliance alert if hash mismatch detected
    - Document itself remains immutable - auditor creates alert, not modifying document
    """
...
def flag_document(
    ...
    """
    Create compliance alert for a document (Auditor read-only operation).
    
    - **AUDITOR** only
    - Creates compliance alert (document remains immutable)
    - Adds ledger entry recording the alert creation
    - Document itself is NOT modified (maintains audit trail integrity)
    - Used when auditor suspects issues or needs to flag for investigation
    """
