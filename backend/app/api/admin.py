...
def list_organizations(
    ...
    results = []
    for org in orgs:
        count = db.query(User).filter(User.org_name == org.org_name).count()
        # Create a dict or object that matches the schema
        org_data = org.__dict__
        org_data['member_count'] = count
