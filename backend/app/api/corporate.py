...
async def upload_document(
    ...
    """
    Submit trade-related documents as Corporate participant.
    
    Corporate users can upload supporting documents for their participation in trades.
    
    RESTRICTIONS (Domain Model):
    - Corporate users CANNOT upload LOC (Letter of Credit)
      → LOC is issued by Bank (operator) to initiate trade
      → Only Bank users can upload LOC
    - Allowed document types: INVOICE, BILL_OF_LADING, PO, COO, INSURANCE_CERT
    
    Risk Scoring:
    - Document upload creates ledger entry with Corporate user as actor
    - This activity factors into their risk score calculation
    """
