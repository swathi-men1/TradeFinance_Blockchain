...
class TradeService:
    ...
    def create_trade(db: Session, current_user: User, trade_data: TradeCreate) -> TradeTransaction:
        ...
        - INITIATED BY: Bank User (the operator/manager)
        - BUYER: Corporate User (participant, subject to risk evaluation)
        - SELLER: Corporate User (participant, subject to risk evaluation)
        
        This keeps:
        - Risk model clean: Only Corporate users are scored
        - Trade finance realistic: Bank initiates, Corporate participates
