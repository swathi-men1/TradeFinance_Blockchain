...
class AuditorService:
    ...
    def verify_document_integrity(db: Session, auditor: User, document_id: int) -> Dict[str, Any]:
        ...
        # Read file from local storage and verify hash
        try:
            if not os.path.exists(document.file_url):
                raise FileNotFoundError(f"Document file not found at {document.file_url}")
            ...
            AuditorService._create_alert(
                ...
                description=f"Document {document.doc_number} file is missing from storage. This may indicate document tampering or deletion.",
                document_id=document.id
            )
            ...
            AuditorService._create_alert(
                ...
                description=f"Cannot verify document {document.doc_number}: {error_message}",
                document_id=document.id
            )

            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to verify document: {error_message}"
            )
    ...
    def flag_document_for_investigation(
        ...
        """
        Create compliance alert for a document (read-only audit operation).
        
        This is a read-only Auditor operation that:
        - Does NOT modify the document itself (maintains immutability)
        - Creates a compliance alert linked to the document
        - Creates a ledger entry recording the alert creation
        - Keeps the audit trail clean and tamper-proof
        """
        document = AuditorService.get_document_by_id(db, document_id)
    ...
    def get_document_file(db: Session, document_id: int) -> Dict[str, Any]:
        """Get document file stream from local storage"""
        document = AuditorService.get_document_by_id(db, document_id)

        if document.file_url.startswith("pending:"):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Document file is pending upload"
            )
        ...
        try:
            ...
            content_type = document.mime_type if hasattr(document, 'mime_type') and document.mime_type else 'application/octet-stream'
            if not content_type or content_type == 'application/octet-stream':
                if filename.lower().endswith('.pdf'):
                    content_type = 'application/pdf'
            
            # Return file iterator and metadata
            def file_iterator():
                with open(document.file_url, 'rb') as f:
