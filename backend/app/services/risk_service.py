...
class RiskService:
    """
    Risk Scoring Service - USER-based risk calculation.
    
    Mentor Requirements:
    1. Risk score is always for a USER (not document)
    ...
    def calculate_user_risk(db: Session, user_id: int, trigger_source: str = "manual") -> Optional[RiskScore]:
        ...
        # Generate short explanation for audit/display
        short_reasons = []
        if doc_risk > 0.1: short_reasons.append(f"Document Integrity Issues ({doc_risk:.0%})")
        if transaction_risk > 0.1: short_reasons.append(f"Transaction Behavior ({transaction_risk:.0%})")
        if activity_risk > 0.1: short_reasons.append(f"Ledger Activity ({activity_risk:.0%})")
        
        if not short_reasons:
            short_explanation = "Low Risk - Good Standing"
        else:
            short_explanation = f"{category} Risk driven by: " + ", ".join(short_reasons)

        # Prepend short explanation to rationale
        rationale_list.insert(0, short_explanation)
        rationale_list.insert(1, "") # Spacer
