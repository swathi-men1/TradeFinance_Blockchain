...
const statusConfig = {
    pending: { color: 'warning', icon: <Clock size={16} />, label: 'Pending' },
    in_progress: { color: 'info', icon: <RefreshCw size={16} />, label: 'In Progress' },
    completed: { color: 'success', icon: <CheckCircle size={16} />, label: 'Completed' },
    paid: { color: 'success', icon: <Banknote size={16} />, label: 'Paid' },
    disputed: { color: 'error', icon: <AlertTriangle size={16} />, label: 'Disputed' },
};
            ...
            <div className="fade-in max-w-6xl">
                ...
                <div className="mb-5">
                    ...
                    <div className="flex flex-col md:flex-row items-start justify-between gap-4">
                        ...
                        <div className={`status-badge status-${config.color} flex items-center gap-2 px-4 py-2 rounded-full`}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center py-3 border-b border-gray-700">
                                ...
                                <div className="text-right">
                                    <p className="text-content-primary font-semibold">{trade.buyer?.name || 'Unknown'}</p>
                                    <p className="text-secondary text-sm">
                                        {trade.buyer?.user_code ? (
                                            <span className="font-mono text-lime-400">{trade.buyer.user_code}</span>
                                        ) : (
                                            `ID: #${trade.buyer_id}`
                                        )}
                                    </p>
                            ...
                            <div className="flex justify-between items-center py-3 border-b border-gray-700">
                                ...
                                <div className="text-right">
                                    <p className="text-content-primary font-semibold">{trade.seller?.name || 'Unknown'}</p>
                                    <p className="text-secondary text-sm">
                                        {trade.seller?.user_code ? (
                                            <span className="font-mono text-lime-400">{trade.seller.user_code}</span>
                                        ) : (
                                            `ID: #${trade.seller_id}`
                                        )}
                                    </p>
                            ...
                            <div className="flex justify-between items-center py-3 border-b border-gray-700">
                                ...
                                <span className="text-content-primary font-semibold">{trade.currency}</span>
                            </div>
                    ...
                    {canUpdateStatus && (
                        <ElevatedPanel>
                            ...
                            {!showStatusUpdate ? (
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-content-primary mb-2">Select New Status</label>
                                        <select
                            ...
                            <div className="mt-8 pt-8 border-t border-gray-700">
                                <h3 className="text-sm font-semibold text-content-primary mb-4">Trade Lifecycle</h3>
                                <div className="space-y-2">
                                    {Object.entries(statusConfig).map(([key, cfg], index) => {
                                        ...
                                        return (
                                            <div
                                                key={key}
                                                className={`flex items-center gap-3 p-2 rounded-lg transition-all ${isCurrentStatus ? 'bg-lime-900/20' : ''
                                                    }`}
                                            >
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isCurrentStatus ? 'bg-lime-500 text-black' :
                                                    isPastStatus ? 'bg-green-900/40 text-green-500' :
                                                        'bg-gray-800 text-gray-500'
                                                    }`}>
                                                    {isPastStatus ? <CheckCircle size={14} /> : cfg.icon}
                                                </div>
                                                <span className={`text-sm ${isCurrentStatus ? 'text-lime-400 font-semibold' :
                                                    isPastStatus ? 'text-green-500' :
                                                        'text-muted'
                                                    }`}>
                                                    {cfg.label}
                                                </span>
                    ...
                    {documents.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {documents.map((doc) => (
                                <div
                                    key={doc.id}
                                    className="panel-surface group hover:border-lime-500 transition-all relative border border-gray-700"
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="flex-1">
                                            ...
                                            >
                                                <p className="font-semibold text-content-primary group-hover:text-lime-400 transition-colors">
                                                    {doc.doc_number}
                                                </p>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <div className="text-secondary group-hover:text-lime-400 transition-colors cursor-pointer"
                                                onClick={() => navigate(`/documents/${doc.id}`)}>
                                                <ArrowRight size={20} />
                                            </div>
                                            {user?.role === 'admin' && (
                ...
                {showDocumentManagement && user?.role === 'admin' && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                        <div className="panel-elevated max-w-2xl w-full max-h-[80vh] overflow-y-auto">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-xl font-bold text-content-primary">Manage Trade Documents</h3>
                                <button
                                    onClick={() => {
                                        setShowDocumentManagement(false);
                                        setSelectedDocuments([]);
                                        ...
                                        {allDocuments.map((doc) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-content-primary font-medium">{doc.doc_type}</span>
                                                            <span className="text-xs text-muted">#{doc.id}</span>
                                                            {isLinked && (
                                                                <span className="text-xs text-success flex items-center gap-1"><Check size={10} /> Linked</span>
                                                            )}
                                                        </div>
                                                        <div className="text-xs text-secondary mt-1">
                                                    ...
                                                    <div className="text-xs">
                                                        <span className={`badge ${doc.status === 'verified' ? 'status-tag-success' : 'status-tag-warning'}`}>
                                                            {doc.status}
                                                        </span>
                                                    </div>
                                                </div>
                                    ...
                                    {selectedDocuments.length > 0 && (
                                        <div className="pt-4 border-t border-gray-700">
                                            <p className="text-sm text-secondary mb-4">
                                                <span className="text-lime-400 font-medium">{selectedDocuments.length}</span> document(s) selected
                                            </p>
                                            <div className="flex gap-3">
                                                <Button
                                                    variant="secondary"
